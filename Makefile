...
example_data: data/bird_move.csv data/zooplankton_example.csv
				R --slave -e "source('code/bird_example_data.R')"
				R --slave -e "source('code/cleaning_zooplankton_data.R')"
...
fig: figures/Fig1.pdf code/alternate_models.R 
				R --slave -e "source('code/alternate_models.R')"
...
main: paper_sections/full_document.Rmd
				R --slave -e "library(knitr); purl('paper_sections/full_document.Rmd', documentation = 0, output = 'compiled_paper/supplemental_code.R')"
				R --slave -e "library(rmarkdown); render('paper_sections/full_document.Rmd', output_file = 'full_document.tex')"
				sed -i.bak '/xcolor/d' paper_sections/full_document.tex
				R --slave -e "setwd('paper_sections'); library(tools); texi2pdf('full_document.tex',clean = TRUE)"
